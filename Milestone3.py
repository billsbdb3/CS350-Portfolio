            ...
            for word in wordList:
                ...
                for char in word:
                    ...
                    for x in morse:
                        ## Dot or dash?
                        if x == '.':
                            self.doDot()   # off -> dot (turn on red LED and wait 0.5s)
                            self.doDot()   # dot -> off (turn red LED off)
                        elif x == '-':
                            self.doDash()  # off -> dash (turn on blue LED and wait 1.5s)
                            self.doDash()  # dash -> off (turn blue LED off)

                        # If there is another symbol in the letter, process a dotDashPause event
                        if morseCounter < lenMorse:
                            self.doDDP()   # off -> dotDashPause (pause 250ms)
                            self.doDDP()   # dotDashPause -> off
                            morseCounter += 1

                    # If there is another letter in the word, process a letterPause event
                    if wordCounter < lenWord:
                        self.doLP()   # off -> letterPause (pause 750ms)
                        self.doLP()   # letterPause -> off
                        wordCounter += 1

                    # If there is another word in the message, process a wordPause event
                    if wordsCounter < lenWords:
                        self.doWP()   # off -> wordPause (pause 3000ms)
